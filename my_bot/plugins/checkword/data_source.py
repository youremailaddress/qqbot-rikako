...
class CheckWordDBHandler(DbHandler):
    ...
    def check_checkword(self,uid,checkwd):
        ...
        if not self.session.query(CheckWord).filter(CheckWord.user==uid,CheckWord.isused=="0",CheckWord.check==checkwd,int(time.time())-int(CheckWord.updatetime)<5*60).count():
            return False
        else:
            cw = self.session.query(CheckWord).filter(CheckWord.user==uid,CheckWord.isused==False,CheckWord.check==checkwd,int(time.time())-int(CheckWord.updatetime)<5*60).one()
